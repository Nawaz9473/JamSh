...
async function runVerification() {
  console.log('----------------------------------------------------');
  console.log('JAMSH REELS PIPELINE PRODUCTION BUILD TEST RUNNER');
  console.log('----------------------------------------------------');
  
  try {
    // Dynamic import to allow global WebSocket polyfill to execute first
    const { fetchReelsFeed, likeReel, commentOnReel, trackVideoInteraction, uploadReel, mockDb, useAuthStore } = await import('../dist/index.js');
    ...
    // Set active user state correctly on the actual Zustand auth store
    useAuthStore.getState().setSession(
      { id: 'user_1', username: 'alex' },
      { id: 'user_1', username: 'alex', display_name: 'Alex', avatar_url: null, cover_url: null, bio: null, website: null, followers_count: 0, following_count: 0, is_private: false, is_verified: false, birthday: null, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
    );
    ...
    const newVideo = await uploadReel(
      ...
      'Summer breeze and sea salt keys. 🌊 #ocean #relaxation',
      ['ocean', 'relaxation'],
      ['Nature'],
      12.4
    );
