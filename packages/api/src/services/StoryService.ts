...
export class StoryService {
   ...
   */
  public static getCachedTray(): StoryTrayItem[] {
    try {
  ...
  private static setCachedTray(tray: StoryTrayItem[]) {
    try {
      if (typeof localStorage !== 'undefined') {
   ...
   */
  static async getStoryTray(): Promise<StoryTrayItem[]> {
    ...
    if (!user) return this.getCachedTray();
    ...
    if (storiesErr || !activeStories) return this.getCachedTray();

    const groupedMap = new Map<string, StoryTrayItem>();
   ...
   */
  static async getUserStories(targetUserId: string): Promise<StoryItem[]> {
    this.initOfflineSync();
    ...
    return stories.map((story: any) => {
      const isViewed = user ? (story.story_views || []).some((v: any) => v.user_id === user.id) : false;
      const myReaction = user ? (story.story_reactions || []).find((r: any) => r.user_id === user.id) : null;
   ...
   */
  static async createStory(payload: CreateStoryPayload): Promise<StoryItem> {
    this.initOfflineSync();
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (attempts >= maxAttempts && error) throw error;
      } catch (e) {
   ...
   */
  static async markStoryViewed(storyId: string): Promise<void> {
    this.initOfflineSync();
   ...
   */
  static async reactToStory(storyId: string, reactionType: string): Promise<{ reaction: string | null; action: 'added' | 'removed' }> {
    this.initOfflineSync();
   ...
   */
  static async getStoryViewers(storyId: string): Promise<StoryViewerItem[]> {
    try {
   ...
   */
  private static enqueueOfflineAction(action: QueuedOfflineAction) {
    try {
      if (typeof localStorage === 'undefined') return;
      const existingStr = localStorage.getItem(QUEUE_CACHE_KEY);
      const queue: QueuedOfflineAction[] = existingStr ? JSON.parse(existingStr) : [];
      queue.push(action);
  ...
  public static async flushOfflineQueue() {
    try {
      ...
      const queue: QueuedOfflineAction[] = JSON.parse(existingStr);
