...
export const StoryPlayer: React.FC<StoryPlayerProps> = ({
  ...
  // Memory management: compute bounded window (previous, current, next)
  ...
  // Image Prefetching & Video Preloading for next story
  useEffect(() => {
    if (!nextStory) return;
    if (nextStory.media_type === 'image' && typeof Image !== 'undefined') {
      try {
  ...
  // Load user stories when authorId changes
  useEffect(() => {
    ...
    const loadStories = async () => {
      try {
        ...
        console.error('[StoryPlayer] Error loading user stories:', e);
      } finally {
  ...
  // Mark story as viewed & animate progress bar
  useEffect(() => {
  ...
  const handleReact = async (emoji: string) => {
    ...
    try {
      ...
      console.error('[StoryPlayer] React error:', e);
    }
  };
  ...
  const handleLoadViewers = async () => {
    ...
    try {
      ...
      console.error('[StoryPlayer] Error fetching viewers:', e);
    } finally {
  ...
  const handleDeleteStory = async () => {
    ...
    try {
      ...
      console.error('[StoryPlayer] Delete error:', e);
    }
  };
